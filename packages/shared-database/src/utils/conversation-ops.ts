...
export class ConversationOperations {
  ...
  async getByCustomer(customerId: string): Promise<Conversation[]> {
    ...
    if (error) {
      throw new Error(`Failed to get conversations for customer: ${error.message}`);
    }
  ...
  async getByPhones(customerPhone: string, brandPhone: string): Promise<Conversation | null> {
    const { data, error } = await this.db.conversations
  ...
  async addMessage(data: CreateMessage): Promise<Message> {
    ...
    if (conversationError) {
      throw new Error(`Failed to update conversation stats: ${conversationError.message}`);
    }
  ...
  async getWithCustomer(id: string) {
    ...
    if (error) {
      throw new Error(`Failed to get conversation with customer: ${error.message}`);
    }
  ...
  async getAnalytics(timeRange?: { start: string; end: string }) {
    ...
    analytics.avgMessagesPerConversation = data.length > 0 ? totalMessages / data.length : 0;
